...
async function extractZipUnsafe(zipPath, destDir) {
...
async function extractZip(zipPath, destDir) {
  try {
    return await extractZipUnsafe(zipPath, destDir);
  } catch (error) {
    if (error.code?.startsWith('ZIP_')) throw error;
    if (['ENOENT', 'EACCES', 'EPERM', 'ENOSPC', 'EMFILE', 'ENFILE'].includes(error.code)) throw error;
    throw archiveError('ZIP_INVALID', '压缩包格式无效或文件不完整');
  }
}
